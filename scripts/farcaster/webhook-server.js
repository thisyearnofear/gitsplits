...
async function handleAnalyzeCommand(command, cast) {
  try {
    ...
    return `Analysis of ${command.repo}:\n\n` +
           `- Contributor 1: 50%\n` +
           `- Contributor 2: 30%\n` +
           `- Contributor 3: 20%\n\n` +
...
async function handleCreateCommand(command, cast) {
  try {
    ...
    console.log(`Creating split for repository: ${command.repo} with allocation: ${command.allocation}`);
    
    // Simulate a delay for split creation
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    return `Split created for ${command.repo} with ${command.allocation} allocation!\n\n` +
...
async function handleSplitsCommand(command, cast) {
  try {
    ...
    return `Splits for ${command.repo}:\n\n` +
...
async function replyCast(text, parentHash) {
  try {
    ...
    if (!NEYNAR_API_KEY || !SIGNER_UUID) {
      throw new Error('NEYNAR_API_KEY or SIGNER_UUID not set in environment variables');
    }
