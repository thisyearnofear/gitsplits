#!/bin/bash
# Script to generate a Function Call Access Key (FCAK) for Crosspost using the exact command from the instructions
...
# Run the exact command from the instructions
echo
echo "Running the exact command from the instructions:"
echo "near account add-key $ACCOUNT_ID grant-function-call-access --allowance '0 NEAR' --contract-account-id crosspost.near --function-names '' autogenerate-new-keypair print-to-terminal network-config mainnet sign-with-keychain send"
echo
...
if [ $? -ne 0 ]; then
  echo
  echo "❌ Error: Failed to generate the Function Call Access Key."
