// Script to test searching for tweets without @ symbol
const Twitter2FAClient = require('./twitter2FAClient');
require('dotenv').config();

async function testSearchWithoutAt() {
  try {
    console.log('Testing searching for tweets without @ symbol...');
    ...
    if (authenticated) {
      ...
      console.log('Searching for tweets containing "gitsplits"...');
      
      // Use the search endpoint directly
      const url = `${twitterClient.baseUrlV1}/search/tweets.json?q=gitsplits&count=10&tweet_mode=extended`;
